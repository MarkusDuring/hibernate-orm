...
//	}
	...
	@Override
	public EntityHolder getEntityHolderOrCreateInitializing(
			EntityKey key,
			Object entity,
		...
		if ( oldHolder != null ) {
			...
			if ( oldHolder.entityInitializer != null || oldHolder.entity != null && oldHolder.state != EntityHolderState.ENHANCED_PROXY && ( processingState.getProcessingOptions().getEffectiveOptionalObject() == null || oldHolder.entity != processingState.getProcessingOptions().getEffectiveOptionalObject() ) ) {
				return oldHolder;
			}
			holder = oldHolder;
		}
	...
	public void postLoad(JdbcValuesSourceProcessingState processingState) {
		// todo: think about maintaining a list of holder objects in JdbcValuesProcessingState instead for post load
		if ( entitiesByKey == null ) {
			return;
		}
